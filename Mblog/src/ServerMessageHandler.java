...
public class ServerMessageHandler extends Thread {
	...
	public void handleServerMessage(String currentMessage) {
		...
		case 1:
			BallotPair proposedBallotNumPair = new BallotPair(
					Integer.parseInt(messageParts[4]),
					Integer.parseInt(messageParts[3]));
			// receivePrepareMsg
			this.replica.logger
					.write("Received Prepare Msg for Paxos Instance: "
							+ String.valueOf(paxosId));
			if (paxosId > numEntries - 1) {
				// Create a new one
				this.replica.paxosEntries.add(new Paxos(paxosId,
						this.replica.replicaId, "", this.replica.logger));
				...
				if (!currentPaxos.isDecided) {
					...
					MessageCommunication.sendDecideUnicast(
							this.replica.replicaId, senderReplicaId, paxosId,
							currentPaxos.valueWritten);
				}
			} else {
				// already decided
				currentPaxos = this.replica.paxosEntries.get(numEntries - 1);
				MessageCommunication.sendDecideUnicast(this.replica.replicaId,
						senderReplicaId, paxosId, currentPaxos.valueWritten);
			}
			break;
		case 2:
			// receiveAcktoPrepareMsg
			this.replica.logger
					.write("Received AckToPrepare Msg for Paxos Instance: "
							+ String.valueOf(paxosId));
			// Ongoing paxos Instance
			if (paxosId == numEntries - 1
					&& !this.replica.paxosEntries.get(paxosId).isDecided) {
				currentPaxos = this.replica.paxosEntries.get(paxosId);
				BallotPair ownBallotNumPair = new BallotPair(
						Integer.parseInt(messageParts[4]),
						Integer.parseInt(messageParts[3]));
				BallotPair acceptedBallotNumPair = new BallotPair(
						Integer.parseInt(messageParts[6]),
						Integer.parseInt(messageParts[5]));
				
				currentPaxos.onreceiveAckToPrepare(ownBallotNumPair,
						acceptedBallotNumPair, messageParts.length > 7 ?messageParts[7]:"");
			} else {
				if (paxosId < numEntries - 1) {
					MessageCommunication
							.sendDecideUnicast(
									...
									senderReplicaId,
									paxosId,
									this.replica.paxosEntries.get(paxosId).valueWritten);
					this.replica.logger
							.write("Responded to  AckToPrepare Msg with Decide Msg for Paxos Instance: "
									+ String.valueOf(paxosId));
				} else {
					this.replica.logger
							.write("Ignoring AckToPrepare Msg for Paxos Instance: "
									+ String.valueOf(paxosId));
				}
			}
			break;
		case 3:
			...
			this.replica.logger
					.write("Received NAckToPrepare Msg for Paxos Instance: "
							+ String.valueOf(paxosId));
			// Ongoing paxos Instance
			if (paxosId == numEntries - 1
					&& !this.replica.paxosEntries.get(paxosId).isDecided) {
				...
				if (paxosId < numEntries - 1) {
					MessageCommunication
							.sendDecideUnicast(
									...
									this.replica.paxosEntries.get(paxosId).valueWritten);
					this.replica.logger
							.write("Responded to  NAckToPrepare Msg with Decide Msg for Paxos Instance: "
									+ String.valueOf(paxosId));
				} else {
					this.replica.logger
							.write("Ignoring NAckToPrepare Msg for Paxos Instance: "
									+ String.valueOf(paxosId));
				}
			}
			break;
		case 4:
			// onreceiveAccept
			BallotPair acceptBallotNumPair = new BallotPair(
					Integer.parseInt(messageParts[4]),
					Integer.parseInt(messageParts[3]));
			this.replica.logger
					.write("Received Accept Msg for Paxos Instance: "
							+ String.valueOf(paxosId));
			if (paxosId > numEntries - 1) {
				// Create a new one
				this.replica.paxosEntries.add(new Paxos(paxosId,
						this.replica.replicaId, "", this.replica.logger));
				currentPaxos = this.replica.paxosEntries.get(numEntries);
				currentPaxos.onreceiveAccept(acceptBallotNumPair,
						messageParts[5]);
			} else if (paxosId == numEntries - 1) {
				// check if it is active or not
				currentPaxos = this.replica.paxosEntries.get(numEntries - 1);
		...
		case 5:
			// onreceiveDecide
			replica.logger.write("Came in ONRECEIVE DECIDE");
			...
			if (paxosId < numEntries - 1) {
				...
				// future stuff in which I did not participate because I was
				// down
				this.replica.logger
						.write("RECVD DECIDE MSG FOR PAXOS INSTANCE "
								+ String.valueOf(paxosId)
		...
		case 7:
			...
			if (highestindex <= num) {
				...
				for (int i = highestindex + 1; i <= num; i++) {
					newValues
							.add(this.replica.paxosEntries.get(i).valueWritten);
				}
				MessageCommunication.replyOnRecover(this.replica.replicaId,
						recoverNodeId, newValues);
			} else {
				this.replica.logger
						.write("RECOVERING NODE KNOWS MUCH MORE THAN ME :(");
			}
			break;
		case 8:
			...
			// handle recovery message from other replicas and set isRecovered =
			// true
			if (!this.replica.isRecovered) {
				synchronized (this.replica.paxosEntries) {

					for (int i = 0; i < messageSize; i++) {
						this.replica.paxosEntries.add(new Paxos(
								highestDecidedIndex + i + 1,
								this.replica.replicaId, messageParts[3 + i]));
					}
				}
